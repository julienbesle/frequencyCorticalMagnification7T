...
function v_rot = rodrigues_rot(v,k,theta)
    ...
    if ( n == 3 )
        k = k./repmat(sqrt(k(:,1).^2 + k(:,2).^2 + k(:,3).^2),1,3); % normalize rotation axis
        crosskv = v(1,:); % initialize cross product k and v with right dim.
        for i = 1:No
            crosskv(1) = k(i,2)*v(i,3) - k(i,3)*v(i,2);
            crosskv(2) = k(i,3)*v(i,1) - k(i,1)*v(i,3);
            crosskv(3) = k(i,1)*v(i,2) - k(i,2)*v(i,1);
            v_rot(i,:) = cos(theta)*v(i,:) + (crosskv)*sin(theta)...
                            + k(i,:)*(dot(k(i,:),v(i,:)))*(1 - cos(theta));
        end
    else % if m == 3 && n ~= 3
        k = k./repmat(sqrt(k(1,:).^2 + k(2,:).^2 + k(3,:).^2),3,1); % normalize rotation axis
        crosskv = v(:,1); % initialize cross product k and v with right dim.
        for i = 1:No
            crosskv(1) = k(2,i)*v(3,i) - k(3,i)*v(2,i);
            crosskv(2) = k(3,i)*v(1,i) - k(1,i)*v(3,i);
            crosskv(3) = k(1,i)*v(2,i) - k(2,i)*v(1,i);
            v_rot(:,i) = cos(theta)*v(:,i) + (crosskv)*sin(theta)...
                            + k(:,i)*(dot(k(:,i),v(:,i)))*(1 - cos(theta));
        end
    end
